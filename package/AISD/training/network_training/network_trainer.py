    ...
    def __init__(self, deterministic=True, fp16=False):
        ...
        if deterministic:
            ...
            if torch.cuda.is_available():                  #cuda是否可用
                torch.cuda.manual_seed_all(12345)       #为所有GPU设置种子用于生成随机数，以使得结果是确定的
            cudnn.deterministic = True
            torch.backends.cudnn.benchmark = False
        else:
            cudnn.deterministic = False
            torch.backends.cudnn.benchmark = True        #设为true，则每次返回的卷积算法将是确定的，即默认算法
        ...
        self.max_num_epochs = 1000                              ##########       最大循环轮数
