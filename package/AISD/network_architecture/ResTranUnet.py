...
import numpy as np
from AISD.network_architecture import CNNBackbone
from AISD.network_architecture.neural_network import SegmentationNetwork
from AISD.network_architecture.DeTrans.DeformableTrans import DeformableTransformer
from AISD.network_architecture.DeTrans.position_encoding import build_position_encoding
...
class U_ResTran3D(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # # skip2 = x_convs[-2]
        # Multi-scale   
        x = self.transposeconv_stage2(x_trans[:, 6400::].transpose(-1, -2).view(x_convs[-1].shape)) # x_trans length: 12*24*24+6*12*12=7776
        skip2 = x_trans[:, 0:6400].transpose(-1, -2).view(x_convs[-2].shape)
