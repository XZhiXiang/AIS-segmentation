...
class PositionEmbeddingSine(nn.Module):              #来源于位置编码计算公式，最终获得独一无二的长度为num_pos_feats的位置编码
    """
    ...
    def forward(self, x):
        ...
        dim_tx = torch.arange(self.num_pos_feats[0], dtype=torch.float32, device=x.device)    #生成一个num_pos_feats[0]维数
        dim_tx = self.temperature ** (3 * (dim_tx // 3) / self.num_pos_feats[0])              #相当于dim_t=10000^{2*(dimt1//3)/d}
