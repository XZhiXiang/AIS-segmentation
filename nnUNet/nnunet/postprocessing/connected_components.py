...
def determine_postprocessing(base, gt_labels_folder, raw_subfolder_name="validation_raw",
    ...
    pp_results['rc_per_class_raw'] = {}
    pp_results['rc_per_class_pp_all'] = {}
    pp_results['rc_per_class_pp_per_class'] = {}

    pp_results['prc_per_class_raw'] = {}
    pp_results['prc_per_class_pp_all'] = {}
    pp_results['prc_per_class_pp_per_class'] = {}
    ...
    for c in classes:
        ...
        rc_raw = validation_result_raw[str(c)]['Recall']

        prc_raw = validation_result_raw[str(c)]['Precision']

        dc_pp = validation_result_PP_test[str(c)]['Dice']

        rc_pp = validation_result_PP_test[str(c)]['Recall']

        prc_pp = validation_result_PP_test[str(c)]['Precision']



        pp_results['dc_per_class_raw'][str(c)] = dc_raw
        pp_results['rc_per_class_raw'][str(c)] = rc_raw
        pp_results['prc_per_class_raw'][str(c)] = prc_raw
        pp_results['dc_per_class_pp_all'][str(c)] = dc_pp
        pp_results['rc_per_class_pp_all'][str(c)] = rc_pp
        pp_results['prc_per_class_pp_all'][str(c)] = prc_pp

    print("validation_result_PP_test:", validation_result_PP_test)

    # true if new is better
    do_fg_cc = False
    comp = [pp_results['dc_per_class_pp_all'][str(cl)] > (pp_results['dc_per_class_raw'][str(cl)] + dice_threshold) for
            cl in classes]
    before_dice = np.mean([pp_results['dc_per_class_raw'][str(cl)] for cl in classes])
    before_recall = np.mean([pp_results['rc_per_class_raw'][str(cl)] for cl in classes])
    before_precision = np.mean([pp_results['prc_per_class_raw'][str(cl)] for cl in classes])
    after_dice = np.mean([pp_results['dc_per_class_pp_all'][str(cl)] for cl in classes])
    after_recall = np.mean([pp_results['rc_per_class_pp_all'][str(cl)] for cl in classes])
    after_precision = np.mean([pp_results['prc_per_class_pp_all'][str(cl)] for cl in classes])

    print("Foreground vs background")
    print("Dice:", dc_raw)
    print("before_dice:", before_dice)
    print("after_dice: ", after_dice)
    print("before_recall", before_recall)
    print("after_racal", after_recall)
    print("before_precision",before_precision)
    print("after_precision", after_precision)
    if any(comp):
